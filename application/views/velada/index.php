...
<style>
 .aumentos{width: 98%!important; background:#F9F9F9!important;height:470px!important;font-size:0;overflow:hidden}.tab-content{padding-top:2px!important;background:transparent}.aumentos h2{margin:10px auto;font-size:18px;font-weight:400}.results-preview{width: 98%!important;background:#F9F9F9}.results-preview h2{padding:8px;margin-top:10px;margin-bottom:5px;font-size:20px}.cotizador-button{margin:10px}.previews{width:20%}.previews input,select{width:90%!important;margin:0 auto}#fecha1{margin-top:13px}.col3{width:33.3%;display:inline-block;vertical-align:top;padding:10px;border-radius:4px:font-size: 2px}.table-container{background:#fff;border-radius:4px;border:solid 1px #ccc}.table-container .login-input{margin-top:0!important}@media all and (max-width : 700px){.previews{width:100%;text-align:center}.cotizador-button{float:none}.col3{width:100%}.aumentos{height:auto!important}.table-container{height: auto!important;}}#resumen{ padding: 20px; }#resumen p{text-align: left;}#resumen span{font-weight: bold}.p-list{margin-left: 70px;} #cancelar,#guardar{margin: 20px!important;}#messages{max-width: 99%!important;}
</style>
...
<form  method="post" id="form-velada"  enctype="multipart/form-data">
  <div class="results-preview" style="margin:0 auto;">
         ...
         <input type="date" id="fecha1" style="font-size: 13px" class="login-input" name="fecha" required placeholder="Fecha">
    ...
    <tbody id="tabla4">
      ...
      <td><select  name="tipo_gasto[]" id="gasto" class="login-input" required>
        <option disabled selected value="">Gastos velada:</option>
         <?php
    $rows5=$optionsmodel->getGastos(); 
    $options_nombre='';
  foreach ($rows5 as $row) { 
  $options_nombre.='<option value="'.$row['tipo_gasto'].'">'.$row['tipo_gasto'].'</option>';                             
   ?>  
  <option value="<?php echo $row['tipo_gasto'];?>"><?php echo $row['tipo_gasto'];?> </option>
  <?php 
    }
   ?>
      </select></td>
